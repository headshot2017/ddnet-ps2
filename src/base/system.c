...
#include <sys/stat.h>
	...
	#include <dirent.h>
		...
		int  lwip_recvfrom(int socket, void *restrict buffer, size_t length, int flags, struct sockaddr *restrict address, socklen_t *restrict address_len);
		int  lwip_sendto(int socket, const void *message, size_t length, int flags, const struct sockaddr *dest_addr, socklen_t dest_len);
...
#if !defined(CONF_PLATFORM_MACOSX) && !defined(_EE)
	if(dbg_msg_threaded)
	{
		int e;
...
int net_host_lookup(const char *hostname, NETADDR *addr, int types)
{
	...
	e = lwip_getaddrinfo(host, NULL, &hints, &result);
...
#else
		dbg_msg("net", "failed to create socket with domain %d and type %d (%d '%s') %d", domain, type, errno, strerror(errno), sock);
#endif
		return -1;
	}
...
NETSOCKET net_udp_create(NETADDR bindaddr)
{
	...
	if(bindaddr.type&NETTYPE_IPV4)
	{
		...
		if(socket >= 0)
		{
			...
			/* set boardcast */
			lwip_setsockopt(socket, SOL_SOCKET, SO_BROADCAST, (const char*)&broadcast, sizeof(broadcast));

			/* set receive buffer size */
			lwip_setsockopt(socket, SOL_SOCKET, SO_RCVBUF, (char*)&recvsize, sizeof(recvsize));

			{
				/* set DSCP/TOS */
				int iptos = 0x10 /* IPTOS_LOWDELAY */;
				//int iptos = 46; /* High Priority */
				lwip_setsockopt(socket, IPPROTO_IP, IP_TOS, (char*)&iptos, sizeof(iptos));
			}
		}
	}
...
int net_udp_send(NETSOCKET sock, const NETADDR *addr, const void *data, int size)
{
	...
	if(addr->type&NETTYPE_IPV4)
	{
		if(sock.ipv4sock >= 0)
		{
			...
			d = lwip_sendto((int)sock.ipv4sock, (const char*)data, size, 0, (struct sockaddr *)&sa, sizeof(sa));
		}
		else
...
int net_udp_recv(NETSOCKET sock, NETADDR *addr, void *data, int maxsize)
{
	...
	if(bytes == 0 && sock.ipv4sock >= 0)
	{
		...
		bytes = lwip_recvfrom(sock.ipv4sock, (char*)data, maxsize, 0, (struct sockaddr *)&sockaddrbuf, &fromlen);
	}
...
#else
		lwip_ioctl(sock.ipv4sock, FIONBIO, (unsigned long *)&mode);
#endif
	}
...
int net_tcp_connect(NETSOCKET sock, const NETADDR *a)
{
	if(a->type&NETTYPE_IPV4)
	{
		...
		return lwip_connect(sock.ipv4sock, (struct sockaddr *)&addr, sizeof(addr));
	}
...
int net_tcp_send(NETSOCKET sock, const void *data, int size)
{
	...
	if(sock.ipv4sock >= 0)
		bytes = lwip_send((int)sock.ipv4sock, (const char*)data, size, 0);
	if(sock.ipv6sock >= 0)
		bytes = lwip_send((int)sock.ipv6sock, (const char*)data, size, 0);
...
int net_tcp_recv(NETSOCKET sock, void *data, int maxsize)
{
	...
	if(sock.ipv4sock >= 0)
		bytes = lwip_recv((int)sock.ipv4sock, (char*)data, maxsize, 0);
	if(sock.ipv6sock >= 0)
		bytes = lwip_recv((int)sock.ipv6sock, (char*)data, maxsize, 0);
...
int net_socket_read_wait(NETSOCKET sock, int time)
{
	...
	if(time < 0)
		lwip_select(sockid+1, &readfds, NULL, NULL, NULL);
	else
		lwip_select(sockid+1, &readfds, NULL, NULL, &tv);
